...
<body class="min-h-screen bg-slate-950 text-white">
    @php
    $platformLoggedIn = Auth::guard('platform')->check();
    $businessLoggedIn = Auth::guard('web')->check();

    if ($platformLoggedIn) {
        $dashboardUrl = route('platform.businesses.index');
        $dashboardLabel = 'Platform dashboard';
        $heroButtonLabel = 'Open platform dashboard →';
    } elseif ($businessLoggedIn) {
        $dashboardUrl = route('admin.dashboard');
        $dashboardLabel = 'Business dashboard';
        $heroButtonLabel = 'Open business dashboard →';
    } else {
        $dashboardUrl = route('login');
        $dashboardLabel = 'Business login';
        $heroButtonLabel = 'Open business dashboard →';
    }
@endphp
    ...
    <main class="relative isolate min-h-screen overflow-hidden">
        ...
        <nav class="mx-auto flex max-w-7xl items-center justify-between px-5 py-6 lg:px-8">
            ...
            <a class="rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm font-bold backdrop-blur hover:bg-white/20" href="{{ $dashboardUrl }}">
    {{ $dashboardLabel }}
</a>
        </nav>
